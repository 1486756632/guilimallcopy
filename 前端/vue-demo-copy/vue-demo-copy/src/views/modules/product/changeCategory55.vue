...
<el-button @click="showSelectPara" type="primary"  display="inline-block" class="btn_search">查询</el-button>
...
dataPara : [],
    ...
    methods:{
      showSelectPara(){
            this.$http({
              url: this.$http.adornUrl('/product/wntparameter/showSelectPara'),
              method: 'post',
              data: this.$http.adornData({
              "ipAndWntPara":this.chosePara+"&"+this.strWntPara              
              })
            }),
            this.$http({
              url: this.$http.adornUrl('/product/wntparameter/getTablePara'),
              method: 'get',
...
this.strWntPara=this.oldSearchJobType
        console.log("输出选择结果     "+this.strWntPara)
      },
      selectData() {
...
handleEdit(row, it) {
    ...
    loadExcel(){
      ...
      var urlDown = 'http://localhost:88/api/product/wntparameter/downContextExcel'
      window.open(urlDown);
      console.log("下载完成")
    },
...
console.log("侧卧式   "+new Date().getTime());
var dateUploadContext=new Date().getTime();
 this.$http({
...
//       });
console.log("测试sleep钱")
//javascript 延时函数
setTimeout("","100000")
this.$options.methods.loadExcel()
  // setTimeout("this.$options.methods.loadExcel()","5000");
  //  var urlDown = 'http://localhost:88/api/product/wntparameter/downContextExcel'
  //     window.open(urlDown);
// this.$http({
//               url: this.$http.adornUrl('/product/wntparameter/downContextExcel'),
//               method: 'get',
//             })
